...
# in an installed env file are preserved on reinstall.
RESUME_WAIT_SECONDS="30"
RESUME_SETTLE_SECONDS="3"
RESUME_REAPPLY_COUNT="2"
RESUME_REAPPLY_INTERVAL_SECONDS="2"
...
preserve_resume_settings() {
    ...
    while IFS='=' read -r key value; do
        ...
        if [ "$key" = "RESUME_REAPPLY_COUNT" ] && [ "$value" -lt 1 ]; then
            echo "Ignoring invalid RESUME_REAPPLY_COUNT in existing env file: $value" >&2
            continue
        fi

        case "$key" in
          RESUME_WAIT_SECONDS) RESUME_WAIT_SECONDS=$value ;;
          RESUME_SETTLE_SECONDS) RESUME_SETTLE_SECONDS=$value ;;
          RESUME_REAPPLY_COUNT) RESUME_REAPPLY_COUNT=$value ;;
          RESUME_REAPPLY_INTERVAL_SECONDS) RESUME_REAPPLY_INTERVAL_SECONDS=$value ;;
        esac
    done < "$ENV_PATH"
}
...
write_env_file() {
    {
        ...
        printf 'RESUME_REAPPLY_INTERVAL_SECONDS="%s"\n' "$RESUME_REAPPLY_INTERVAL_SECONDS"
    } > "$ENV_PATH"
    chmod 0644 "$ENV_PATH"
